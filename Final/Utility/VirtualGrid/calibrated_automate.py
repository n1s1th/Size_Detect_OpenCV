...
corners_file = "C:\\Users\\Dell\\Desktop\\Projects\\Size_Detect_OpenCV\\Final\\Utility\\VirtualGrid\\grid_corners.json" # File to store/load calibration vectors
...
def crop_grid_area(frame, pts_src):
    rect = cv2.boundingRect(np.array(pts_src, dtype="int32"))
    x, y, w, h = rect
    return frame[y:y+h, x:x+w]

def load_corners(filename):
    if not os.path.exists(filename):
        print(f"Calibration file '{filename}' not found. Please run calibration first.")
        return None
...
def main():
    ...
    if corners is None or len(corners) != 4:
        print("Calibration incomplete or file missing.")
        return
    ...
    while True:
        ...
        # Optionally crop to the grid area (use frame_with_grid for overlay)
        # cropped = crop_grid_area(frame, pts_src)
        frame_with_grid = draw_grid_and_labels(frame.copy(), matrix, grid_w, grid_h, num_cells_x, num_cells_y)
        cv2.imshow("Virtual Grid (Auto Alignment)", frame_with_grid)
